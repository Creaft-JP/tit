...
// The GlobalConfigFunc type is an adapter to allow the use of ordinary
// function as GlobalConfig mutator.
type GlobalConfigFunc func(context.Context, *ent.GlobalConfigMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GlobalConfigFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GlobalConfigMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GlobalConfigMutation", m)
}
