...
func (cq *CommitQuery) QueryFiles() *CommittedFileQuery {
	query := (&CommittedFileClient{config: cq.config}).Query()
...
func (cq *CommitQuery) WithFiles(opts ...func(*CommittedFileQuery)) *CommitQuery {
	query := (&CommittedFileClient{config: cq.config}).Query()
...
func (cq *CommitQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Commit, error) {
	...
	if query := cq.withFiles; query != nil {
		if err := cq.loadFiles(ctx, query, nodes,
			func(n *Commit) { n.Edges.Files = []*CommittedFile{} },
			func(n *Commit, e *CommittedFile) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommitQuery) loadFiles(ctx context.Context, query *CommittedFileQuery, nodes []*Commit, init func(*Commit), assign func(*Commit, *CommittedFile)) error {
	...
	query.Where(predicate.CommittedFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(commit.FilesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.commit_files
		if fk == nil {
			return fmt.Errorf(`foreign-key "commit_files" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "commit_files" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
