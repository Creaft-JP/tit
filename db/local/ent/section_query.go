...
func (sq *SectionQuery) Order(o ...section.OrderOption) *SectionQuery {
	sq.order = append(sq.order, o...)
	return sq
}

// QueryPage chains the current query on the "page" edge.
func (sq *SectionQuery) QueryPage() *PageQuery {
	query := (&PageClient{config: sq.config}).Query()
...
func (sq *SectionQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (sq *SectionQuery) WithPage(opts ...func(*PageQuery)) *SectionQuery {
	query := (&PageClient{config: sq.config}).Query()
...
func (sq *SectionQuery) loadPage(ctx context.Context, query *PageQuery, nodes []*Section, init func(*Section), assign func(*Section, *Page)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Section)
	for i := range nodes {
		if nodes[i].page_sections == nil {
			continue
		}
		fk := *nodes[i].page_sections
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "page_sections" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
