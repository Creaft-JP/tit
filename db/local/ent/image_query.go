...
func (iq *ImageQuery) QueryCommit() *TitCommitQuery {
	query := (&TitCommitClient{config: iq.config}).Query()
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withCommit: iq.withCommit.Clone(),
...
func (iq *ImageQuery) WithCommit(opts ...func(*TitCommitQuery)) *ImageQuery {
	query := (&TitCommitClient{config: iq.config}).Query()
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withCommit; query != nil {
		if err := iq.loadCommit(ctx, query, nodes,
			func(n *Image) { n.Edges.Commit = []*TitCommit{} },
			func(n *Image, e *TitCommit) { n.Edges.Commit = append(n.Edges.Commit, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadCommit(ctx context.Context, query *TitCommitQuery, nodes []*Image, init func(*Image), assign func(*Image, *TitCommit)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(image.CommitTable)
		s.Join(joinT).On(s.C(titcommit.FieldID), joinT.C(image.CommitPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(image.CommitPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(image.CommitPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*TitCommit](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "commit" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
